...
async function getTestimony() {
    const res = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/testimonial`, {
        headers: {
            'Cache-Control': 'no-cache',
        }
    });
...
export default async function HomePage() {
    ...
    try {
        const [companyRes, testimonyRes] = await Promise.all([
            getCompanyData(),
            getTestimony(),
        ]);

        companyData = companyRes;
        testimonials = testimonyRes;
    } catch (error) {
        ...
        <>
            <section className="relative text-center py-20 text-white overflow-hidden">
                <div className="absolute inset-0 z-0">
                    ...
                    <div
                        className="absolute inset-0 bg-gradient-to-t from-transparent via-transparent to-black/40 blur-3xl"/>
                </div>
